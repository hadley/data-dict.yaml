...
json_path <- function(name) file.path("site", "examples", paste0(name, ".json"))
...
write_qmd <- function(name, repo) {
  writeLines(
    c(
      ...
      sprintf('json: "[Download](%s.json)"', name),
      "---",
      "",
...
sync <- function(name, repo, src) {
  ...
  if (ok) {
    ...
    # export-spec is the fully-resolved dictionary; export-data would also
    # embed the data profiles, which the rendered page already carries.
    bin <- Sys.getenv("DATA_DICT", "target/release/data-dict")
    system2(bin, c("export-spec", "--pretty", dict), stdout = json_path(name))
  } else {
    cat(sprintf(
