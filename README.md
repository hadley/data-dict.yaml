...
There have been many previous attempts to encode data dictionaries in structured text. What makes `data-dict.yaml` special and why did we decide to revisit this problem?
...
* The cost of describing the semantics of a dataset in two places (i.e. `data-dict.yaml` and data transformation code) is lower because an AI agent can easily keep both in sync.
...
* A toolkit for accurately describing the full **data cleaning** journey. `data-dict.yaml` is primarily useful for describing the mostly-clean mostly-tidy datasets at the end of a data engineering pipeline. 

* `data-dict.yaml` is not a **data validation** tool. You can check that the spec and the data are aligned, but it’s up to you to fix inconsistencies to ensure that your spec has not deviated from reality.

* `data-dict.yaml` is not a **[semantic model](semantic-models.md)**. It doesn’t classify columns as dimensions or metrics, because that distinction reflects intended use, not the data itself. Data scientists need to understand what the data means, not have views predefined for them.

There are other things that `data-dict.yaml` doesn't do **yet**, but are on the roadmap:

* **Large tables**: Standalone `data-dict.yaml` are not designed for hundreds of tables or hundreds of columns. Our plan is to also provide a service that allows you to index much larger data catalogs.
