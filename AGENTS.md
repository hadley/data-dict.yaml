...
- `tools/pypi/`: `make_wheels.py` repacks the release archives into PyPI wheels (one per target, carrying the prebuilt binary as a wheel script), run from `.github/workflows/publish-pypi.yml`, which dist calls as a custom publish job. Stdlib only; test with `python3 -m unittest discover -s tools/pypi`.
