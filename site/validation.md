...
| S32 | Definition shadows column | E | A definition's `name` matches a column name in the same table; definitions and columns share a namespace, so a reference to that name would be ambiguous. |
| S33 | Circular definition reference | E | Following the definition references from a definition leads back to the definition itself. |
